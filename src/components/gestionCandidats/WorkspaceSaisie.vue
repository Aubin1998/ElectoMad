            ...
            <div class="scroll-container border-t-2">
                <div class="item" v-for="(item, index) in filteredElecteurs" :key="index">
                    ...
                    <div class="flex justify-around px-2 w-[15%]">
                        ...
                        <div class="icon orange" @click="modifier(item)" v-if="role == 'admin' || role == 'Controleur'">
                            <i class="pi pi-pencil" style="font-size: 18px; color: white;"></i>
                        </div>
                        <div class="icon red" @click="supprimer(item)" v-if="role == 'admin' || role == 'Controleur'">
...
<script setup>// Filtrer les électeurs par nom complet basé sur la recherche
...
function see() {
    show.showModallAddElecteur = true
}

watch(listeElecteur.listElecteurByFOnkontany, (newList, oldList) => {
    role.value = JSON.parse(localStorage.getItem('usersRole'))
...
function getListElecteur() {
    ...
    listeElecteur.filterElecteurByFonkontany(annee_electorale_id, region, commune, district, fokontany)


    setTimeout(() => {
        console.log(listeElecteur.listElecteurByFOnkontany);
    }, 3000);
}
...
const filteredElecteurs = computed(() => {
    return listeElecteur.listElecteurByFOnkontany.filter(electeur =>
